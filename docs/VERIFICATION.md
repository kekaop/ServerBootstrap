...
Проверка выполнена 18 сентября 2026 на Windows.
...
- На каждой комбинации: 91 тест, 0 ошибок, 0 пропусков.
...
XML/JSON из последнего прогона находятся в build/compatibility при локальной сборке; в поставке также приложен verification.zip. Подробный повторяемый порядок: TESTING.md.
